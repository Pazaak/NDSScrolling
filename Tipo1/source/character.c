...
#define c_ground_friction   15				 // friction when the character hits the ground, multiply X by (256-f)/256
...
#define min_height		  (1200)			 // the minimum height of the character (when it gets squished) (*.8)
...
//-----------------------------------------------------------------
{
	/** Evita que el personaje se salga por los lados */
	if( b->x >= 0 && b->x <= 61000 ){
	...
	if( b->y >= c_platform_level )
	{
		...
		// check if the character has been squished to minimum height
		if( b->y > c_platform_level )
		{
			// mount Y on platform
...
//-----------------------------------------------------------------
// Actualiza el movimiento de la pelota sin considerar velocidades
// ni la gravedad.
//-----------------------------------------------------------------
